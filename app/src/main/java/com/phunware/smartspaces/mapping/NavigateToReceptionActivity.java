...
import com.phunware.smartspaces.network.MockNetWorkManagerImpl;
...
public class NavigateToReceptionActivity extends AppCompatActivity implements
    ...
    private static final String TAG = NavigateToReceptionActivity.class.getSimpleName();
        ...
        try {
            ...
            phunwareMap.getGoogleMap().animateCamera(cameraUpdate, new GoogleMap.CancelableCallback() {
                ...
                public void onFinish() {
                    ...
                    phunwareMapManager.addFloorChangedListener(NavigateToReceptionActivity.this);
                        ...
                        public void run() {
                            waitToAcquireLocation(new BlueDotAcquisitionListener() {
                                ...
                                public void onBlueDotAcquired() {
                                    ...
                                    RouteOptions route = findRouteFromCurrentLocationToPoi("Reception");
    ...
    private void showErrorAndFinish(String errorMessage) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(NavigateToReceptionActivity.this, errorMessage, Toast.LENGTH_LONG).show();
            }
        });
    ...
    public void onManeuverChanged(Navigator navigator, int position) {
        ...
        for(int i=currentManeuverIndex; i < navigator.getManeuvers().size(); ++i){
            List<RouteManeuverOptions> maneuvers = navigator.getManeuvers();
            for(RouteManeuverOptions maneuver: maneuvers){
                distance+= maneuver.getDistance();
            }
        }

        int lastIndex = navigator.getManeuvers().size() - 1;
        if(currentManeuverIndex == lastIndex){
            MessageDialogFragment.newInstance("You have arrived at reception",
                    "Please pick up your badge and proceed to hacking area")
